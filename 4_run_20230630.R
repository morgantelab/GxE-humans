...
#args <- c(14, 3, 11,  1100, 100, 10,   100, 4)
args
...
if(args[2]==1){
  ...
  preds	<- data.frame(ID=dt$ID, sub=dt$sub, coh=dt$coh, phen=Phen, y=model$y, yHat=model$yHat)
...
if(args[2]==4){
  ...
  B1 <- read.table(paste(where, '_output/run_', args[1], '_', args[2], '_', args[3], '_ETA_X_b.dat', sep=''), header=T)
  B2 <- read.table(paste(where, '_output/run_', args[1], '_', args[2], '_', args[3], '_ETA_G_b.dat', sep=''), header=T)
  B3 <- data.frame(matrix(0, nrow(zz0), length(Evars)))
