...
#args <- c(14, 30, 1,  1100, 100, 10,   100, 6)
args
...
if(args[2]==132){
	load("../datasets/eG99AA_20231229.RData")
	GAAmat <- eGAA$vectors
	for(i in 1:ncol(GAAmat)){GAAmat[,i] <- GAAmat[,i]*eGAA$values[i]}
	eGAAvars <- colnames(GAAmat) <- paste("GAA", 1:ncol(GAAmat), sep='.')
	print(length(which(dt$ID!=eGAA$ids)))
	rm(eGAA); gc()
}
...
Amat <- as.matrix(dt[Avars]); dim(Amat); NAcol(Amat)
...
Zmat <- Xmat
if(args[3]==103){
 Zmat[,which(colnames(Zmat)=="coh03")] <- 0
 Zmat[,which(colnames(Zmat)=="coh04")] <- 0
 Zmat[,which(colnames(Zmat)=="coh13")] <- 0
 Zmat[,which(colnames(Zmat)=="coh14")] <- 0
} 
...
if(args[2]==1){
  ...
  rm(Xmat, Zmat, Emat, Amat, Gmat, EL2mat, Ee2mat); gc()
...
#  BETA[[2]] <- B2
  BETA[[3]] <- B3
  if(args[3]!=0){save(BETA, file=paste(paste(where, '_results/BETA_', args[1], '_', args[2], '_', args[3], '.RData', sep='')))}
...
if(args[2]==8){
  ...
  ETA <- list(X=list(X=Xmat, model="FIXED", saveEffects=TRUE), 
   ...
   rm(Xmat, Zmat, Emat, Amat, Gmat, EL2mat, Ee2mat, GEEmat); gc()
...
if(args[2]==9){
  ...
  ETA <- list(X=list(X=Xmat, model="FIXED", saveEffects=TRUE), 
   ...
   rm(Xmat, Zmat, Emat, Amat, Gmat, EL2mat, Ee2mat, GEL2EL2mat); gc()
...
if(args[2]==10){
  ...
  ETA <- list(X=list(X=Xmat, model="FIXED", saveEffects=TRUE), 
   ...
   rm(Xmat, Zmat, Emat, Amat, Gmat, EL2mat, Ee2mat, GEe2Ee2mat); gc()
...
if(args[2]==32){
  ...
  ETA <- list(
              G=list(X=Gmat, model="BRR", saveEffects=TRUE), 
              E=list(X=Amat, model="BRR", saveEffects=TRUE)
  )
   rm(Xmat, Zmat, Emat, Amat, Gmat, EL2mat, Ee2mat); gc()
...
if(args[2]%in%c(120)){
  ...
  ETA <- list(X=list(X=Zmat, model="FIXED", saveEffects=TRUE), 
              G=list(X=Gmat, model="BRR", saveEffects=TRUE), 
              E=list(X=Emat, model="BRR", saveEffects=TRUE),
              GE=list(X=GEEmat, model="BRR", saveEffects=TRUE)
  )
...
if(args[2]%in%c(132)){  
  ...
  ETA <- list(G=list(X=Gmat, model="BRR", saveEffects=TRUE), 
              E=list(X=Amat, model="BRR", saveEffects=TRUE),
              GE=list(X=GAAmat, model="BRR", saveEffects=TRUE)
  )
   rm(Xmat, Zmat, Emat, Amat, Gmat, EL2mat, Ee2mat, GAAmat); gc()
